...
$stmt = $db->prepare('INSERT INTO TaskCmds ("ClientID", "ELID", "StatusesToUpdate", "NewStatus", "Users", "Minutes", "Hours", "WeekDays", "MonthDays", "Months")
     VALUES (:client_id, "CREATING", :statuses_to_update, :new_status, :users, :minutes, :hours, :week_days, :month_days, :months)');
$stmt->bindValue(':client_id', $client['ID'], SQLITE3_INTEGER);
$stmt->bindValue(':statuses_to_update', $data['statuses_to_update'], SQLITE3_TEXT);
...
$cmd_id = $db->querySingle('SELECT * FROM TaskCmds WHERE ELID = "CREATING"');
if (! $cmd_id) {
    $db->exec('ROLLBACK');
    fatal_with_http_header($db->lastErrorMsg(), 500);
}
...
$req = [
    ...
    'command' => "GET https://bg59.online/Apps/tasks_market_dev/api/cron/tasks.php?id={$cmd_id}",
    'active' => 'on',
    'hideout' => 'on',
    'func' => 'scheduler.edit',
    'sok' => 'ok',
...
$stmt = $db->prepare('UPDATE TaskCmds SET ELID = :elid WHERE ID = :id');
